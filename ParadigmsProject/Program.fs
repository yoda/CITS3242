...
let rec unify exp1 exp2 sublist = 
    ...
    |Some(sl) ->                    //A sublist, so we can continue
        ...
        | Some(a), Some(aa) ->  //Both vars are in the substitution list. This means that we need to check they have the same mapping, else a problem may have occured earlier.
            if a = aa then
                //sublist         //Have the same mapping. Because that mapping was generated by a unify call, it is already fully mapped.
                unify a aa sublist
            else
...
let rec GetValidRuleList rules subgoal (substitutionList: (exp * exp) list) checkSubGoalsSucceedFunc =
    [for rule in rules do                                                           //For each rule
                ...
                |Rule((r1, r2), subgoals), (p1,p2) ->                                        //Pattern match the rule
                    ...
                    |None -> ()                                                     //No Match
                    |Some(sl) ->         //A Match!
                        prRaw -1 (sprintf "Matched [[%s suffices %s]] with %O" (getExperimentAsStringWithSubs p1 (Some(sl))) (getExperimentAsStringWithSubs p2 (Some(sl))) rule) |> ignore          
                        match subgoals with
                        |[] -> yield sl                                             //No Subgoals, return subList as is
                        |_ -> yield! checkSubGoalsSucceedFunc rules subgoals sl]
